<template>
    <div class="bg-white rounded shadow w-2/3 mt-6 overflow-hidden">
        ...
        <div v-if="comments" class="border-t border-gray-400 p-4 pt-2">
            <div class="flex">
                <input v-model="commentBody" type="text" name="comment" placeholder="Write your comment"
                       class="w-full pl-4 h-8 bg-gray-200 rounded-lg focus:outline-none">
                <button v-if="commentBody" class="bf-gray-200 ml-2 px-2 py-2 rounded-lg focus:outline-none"
                        @click="postNewComment($vnode.key)">
                    Post
                </button>
            </div>
            <div class="flex my-4 items-center" v-for="comment in post.data.attributes.comments.data">
                ...
                <div class="ml-4 flex-1">
                    <div class="bf-gray-200 rounded-lg p-2 text-sm">
